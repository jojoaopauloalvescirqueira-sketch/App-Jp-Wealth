...
- Alladin: `docs/architecture/ALLADIN.md` — contrato do agregado `S.alladin` (schema **v5**, quatro entidades cadastrais + ledger econômico, dinheiro em unidade mínima, write gate transacional, fail-closed). Código em `src/js/10-domain/13-alladin.js` (domínio, cadastral E econômico), `src/js/20-ui/24-alladin-views.js` (superfície cadastral do C3) e `src/js/00-core/04-persistence.js`; suítes `tools/alladin_*_test.py`. Spec canônica: JPW-ALLADIN-SPEC V1.2.1 (vault de arquitetura, externa ao repo). **Existente**: o C3 — cadastro — CONCLUÍDO (leitura, criação, edição e `recordStatus` das quatro entidades pela interface real); o **ledger ALD-03 S1/S2/S3 PUBLICADO** — Cash Ledger (`DEPOSIT`/`WITHDRAWAL`/`TRANSFER`/`REVERSAL`), trades (`BUY`/`SELL`, fato único de duas pernas) e **despesas standalone** (`FEE`/`TAX`, só-caixa, sem `flowScope` e sem vínculo a trade), com saldo de caixa derivado fail-closed; e o **Position Quantity Engine ALD-04 S1 PUBLICADO** — posição por quantidade **derivada** via `leitura.posicoes()`, identidade `instrumentId + accountId`, aritmética decimal exata, nada persistido. **Não existente** (fronteira que não se antecipa sem gate humano): `ADJUSTMENT` standalone, holding persistido, consolidated position, cost basis, valuation, P&L, performance e UI transacional/de posições — não reconstruir o que já existe nem antecipar o que não foi autorizado.
