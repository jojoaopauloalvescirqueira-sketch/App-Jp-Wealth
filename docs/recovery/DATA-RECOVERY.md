...
As chaves auxiliares auditadas sao `jpw_rail`, `jpw_expl`, `jpw_fs`, `jpwealth_v9_icon_choice`, a chave legada `jpwealth_v9_icon_theme`, as preferencias isoladas do laboratorio em `jpwealth_galton_preferences_v1` e o sinal temporario `jpwealth_session_wipe_signal_v1`; o checkpoint fica em `sessionStorage` como `jpwealth_session_checkpoint_v1`. A limpeza nao usa `localStorage.clear()`.
