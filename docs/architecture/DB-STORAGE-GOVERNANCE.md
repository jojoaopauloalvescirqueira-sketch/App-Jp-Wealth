...
| AUXILIARY PREFERENCE | perfil/foto, launcher, navegação/layout/ícone, Laboratório e associação local da pasta | Preferências da allowlist são transportadas no bloco opcional `workspace` do Backup Completo; capacidade/handle de pasta continua local e exige autorização |
...
| localStorage · `jpw_rail` | AUXILIARY PREFERENCE | [src/js/20-ui/02-sidebar.js](../../src/js/20-ui/02-sidebar.js:3) leitura/controle da barra; [src/js/00-core/07-workspace-backup.js](../../src/js/00-core/07-workspace-backup.js:2) allowlist workspace | Fora de S; incluído em `workspace.preferences`. | Escolha collapsed/expanded por navegador. |
| localStorage · `jpw_nav_submenu_rail` | AUXILIARY PREFERENCE | [src/js/20-ui/02-sidebar.js](../../src/js/20-ui/02-sidebar.js) largura da Lateral em níveis; [src/js/00-core/07-workspace-backup.js](../../src/js/00-core/07-workspace-backup.js) allowlist/validação | Fora de S; incluído em `workspace.preferences` schemaVersion 1. | `expanded`/`collapsed`; independente de `jpw_rail`; fallback expanded não reescreve raw ausente/inválido; gravação explícita é confirmada por releitura. |
...
| localStorage · `jpw_nav` | AUXILIARY PREFERENCE | [src/js/20-ui/12-nav-style.js](../../src/js/20-ui/12-nav-style.js) navStyleValue/controle da Central; [src/js/00-core/07-workspace-backup.js](../../src/js/00-core/07-workspace-backup.js) allowlist | Fora de S; incluído em `workspace.preferences`. | Kinetic/Pill/Clássico; fica indisponível enquanto Liquid Glass ou Lateral em níveis está ativo, sem perder o raw. |
| localStorage · `jpw_nav_layout` | AUXILIARY PREFERENCE | [src/js/20-ui/12-nav-style.js](../../src/js/20-ui/12-nav-style.js) initNavLayoutChoice/mountNavigationLayout | Fora de S; incluído em `workspace.preferences`. | `sidebar`/`topbar`/`glass`/`submenu`; valor inválido preservado enquanto se exibe fallback lateral. |
| localStorage · `jpw_nav_order` | AUXILIARY PREFERENCE | [src/js/20-ui/12-nav-style.js](../../src/js/20-ui/12-nav-style.js) navOrderRead/ato de save com releitura | Fora de S e do envelope de widgets; incluído em `workspace.preferences`. | Identidades dos mesmos seis primários. |
| localStorage · `jpw_nav_glass_tint` | AUXILIARY PREFERENCE | [src/js/20-ui/12-nav-style.js](../../src/js/20-ui/12-nav-style.js) prévia/confirmação; [src/js/00-core/07-workspace-backup.js](../../src/js/00-core/07-workspace-backup.js) validação/importação | Fora de S; incluído em `workspace.preferences` schemaVersion 1. | String inteira `0..100`; fallback 60 não reescreve raw ausente/inválido; importação inválida é recusada antes de escrita. |
| localStorage · `jpwealth_v9_icon_choice` | AUXILIARY PREFERENCE | [src/js/40-app/06-app-icons.js](../../src/js/40-app/06-app-icons.js:8) marca do cabeçalho e instalação PWA; [src/js/40-app/07-finalize-session.js](../../src/js/40-app/07-finalize-session.js:5) | Excluído de S e backup. | `primary`/Vermelho ou `secondary`/Preto; Finalizar remove. |
...
A enumeração inclui **25 chaves/famílias localStorage identificadas**, um checkpoint sessionStorage, um store IndexedDB e um namespace CacheStorage; as linhas de filesystem/exports/RAM delimitam destinos e exclusões. A chave corrompido é uma família dinâmica. A classe UNRESOLVED final não representa chave encontrada. Preferências da allowlist `JPW_WORKSPACE_KEYS`, incluindo navegação, viajam no workspace do Backup Completo; caches e capacidades locais permanecem excluídos. Importação e Zona de Perigo não devem ser tratados como “limpar todo storage”; observar seus parâmetros e contratos específicos. Nenhum `localStorage.clear()` do produto foi encontrado/introduzido.
...
As extensões schema1 `forex.instrumentContexts`, `forex.dailyReferences` e `forex.accountContexts` pertencem ao documento principal. `mutate` conserva write-before-confirm, epoch e guardas de revisão. Recusa preserva rascunho RAM; UNKNOWN não permite retentativa cega. Cotações não sobrescrevem observações manuais. Backup inclui somente estado confirmado; não inclui rascunhos de linha nem bytes de relatório. Finalizar Sessão preserva contextos, observações e operações confirmados por conta, enquanto encerra processos temporários e desbloqueios. Detalhes em [FOREX-EXECUTION-BOARD.md](FOREX-EXECUTION-BOARD.md); nenhum novo banco, backend ou chave de segredo.
