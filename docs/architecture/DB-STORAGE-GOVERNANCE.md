...
| localStorage · `jpw_nav` | AUXILIARY PREFERENCE | [src/js/20-ui/12-nav-style.js](../../src/js/20-ui/12-nav-style.js) navStyleValue/controle da Central; [src/js/00-core/07-workspace-backup.js](../../src/js/00-core/07-workspace-backup.js) allowlist | Fora de S; incluído em `workspace.preferences`. | Kinetic/Pill/Clássico; fica indisponível enquanto Liquid Glass está ativo, sem perder o raw. |
| localStorage · `jpw_nav_layout` | AUXILIARY PREFERENCE | [src/js/20-ui/12-nav-style.js](../../src/js/20-ui/12-nav-style.js) initNavLayoutChoice/mountNavigationLayout | Fora de S; incluído em `workspace.preferences`. | `sidebar`/`topbar`/`glass`; valor inválido preservado enquanto se exibe fallback lateral. |
