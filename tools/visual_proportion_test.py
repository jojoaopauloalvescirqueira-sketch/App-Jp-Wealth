...
def run(args):
    ...
    if args.stress:
        cases += [(390,'light',1.25,True),(1440,'dark',1.25,True),(1440,'light',2,False)]
    try:
                        ...
                        try:
                            ...
                            if local=='settings':
                                ...
                                if page.evaluate('innerWidth')<=760:
                                    ...
                                    page.locator('[data-settings-category=general]').click()
                            ...
                            if name=='settings':
                                ...
                                fits=page.locator('#settingsTitle').evaluate('''e=>{const a=e.getBoundingClientRect(),b=e.closest('.settings-modal-head').getBoundingClientRect();return a.top>=b.top&&a.bottom<=b.bottom&&a.right<=b.right}''')
                                check(key+' title contained by header',fits)
                                sizes=page.evaluate("() => Object.fromEntries([['headline','.cp-settings-intro h4'],['body','.settings-nav-card-title'],['aux','.settings-nav-card-desc'],['window','#settingsTitle']].map(([k,s])=>[k,parseFloat(getComputedStyle(document.querySelector(s)).fontSize)]))")
                                f=1.25 if scale==1.25 else 1
                                check(key+' contextual headline 20–24',20*f<=sizes['headline']<=24*f,sizes)
                                check(key+' card text readable',sizes['body']>=13*f and sizes['aux']>=12*f,sizes)
                                check(key+' headline subordinate to task',sizes['headline']/sizes['body']<=1.85,sizes)
                                check(key+' window title readable',16*f<=sizes['window']<=20*f,sizes)
                                check(key+' five destinations',page.locator('[data-settings-panel=general] [data-nav-to]').count()==5)
                                aligned=page.evaluate('''() => [...document.querySelectorAll('.cp-settings-collections .settings-nav-card')].map(e=>{const t=e.querySelector('.settings-nav-card-text').getBoundingClientRect(),i=e.querySelector('.cp-settings-symbol').getBoundingClientRect();return Math.abs(t.left-i.right-parseFloat(getComputedStyle(e).gap)*(parseFloat(getComputedStyle(document.documentElement).zoom)||1))<2})''')
                                check(key+' card text follows icon column',all(aligned),aligned)
