...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            def require(value):
                assert value
            test('Consolidado is the first Forex destination',lambda:require(page.evaluate("NAV_FOREX_CHILDREN.map(x=>x.id).join(',')==='forex-consolidated,forex-planning,forex-operation,forex-reconciliation,forex-reserves'")))
