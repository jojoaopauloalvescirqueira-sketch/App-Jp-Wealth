...
# A file:// page cannot import a neighbouring ESM file. The generated classic
# payload is loaded lazily by the PDF adapter, then decoded to local module URLs.
# Its inputs are the same pinned resources already covered by the fingerprint.
embedded_assets = {item['path']: base64.b64encode((ROOT / item['path']).read_bytes()).decode('ascii')
                   for item in RUNTIME_ASSETS}
if embedded_assets:
    local_payload = ROOT / 'src/vendor/pdfjs/runtime-assets.js'
    local_payload.write_text(
        '// Generated by tools/rebuild_monolith.py; do not edit.\n'
        + 'window.JPW_RUNTIME_ASSETS = ' + json.dumps(embedded_assets, separators=(',', ':')) + ';\n',
        encoding='utf-8',
    )
