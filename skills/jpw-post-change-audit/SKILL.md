...
Report findings before summary. Include exact commands and classifications, changed functions/contracts, untested areas, residual risk and the next human gate. Do not call the candidate ready if a required `PRODUCT_FAIL`, `ENVIRONMENT_ERROR` or `NOT_RUN` remains unexplained. The only exception for a historical `PRODUCT_FAIL` is a formal `HISTORICAL_UNRESOLVED_FAILURE` finding accepted by an independent auditor under the canonical Harness §19.1 **after its activation record has been verified**; the physical presence of that section is not activation. The raw result remains `PRODUCT_FAIL` and the maximum audit result is `AUDIT_PASS_WITH_DEBT`. This does not waive `ENVIRONMENT_ERROR`, `NOT_RUN`, Human Acceptance or a later Git authorization. Never commit, push, merge or deploy without separate authorization.
