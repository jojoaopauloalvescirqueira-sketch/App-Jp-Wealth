...
(function initNotificationCenter(){
  ...
  function collectSystem(next){
    ...
    if(typeof dgBackupDue==='function' && dgBackupDue()){
      const days=dgBackupAgeDays();
      add(next,'system:backup','system','warning','Backup recomendado',
        days===null?'Ainda não há confirmação de backup.':days+' dias desde a última confirmação de backup.',
        'backup',String(S.dataGovernance?.backup?.lastConfirmedAt||'never'));
    }
