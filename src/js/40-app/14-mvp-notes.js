...
// ---- ações em massa: exportar e copiar o recorte visível (JPW-436587) ----------------
// PRINCÍPIO: a unidade de operação é o RECORTE, não a pasta. mvpNotesFiltered() já cruza
// pasta ativa + status + tipo + prioridade + política + período + busca — é exatamente o
// que a lista mostra. "Copiar as notas abertas da pasta X" é um caso particular disso
// (filtro de status em "Aberta" dentro da pasta), então não existe segundo mecanismo de
// seleção para manter em sincronia com o primeiro.
//
// Ambas as ações são LEITURA PURA: não gravam, não chamam save(), não registram no
// changeLog e não tocam nota alguma. É deliberado — o backup completo da base tem
// governança própria (sequência, dgLogChange); estas não são backup, são extrato.
//
// CRITÉRIO: leva o backlog ATIVO do recorte (exclui Concluída e Descartada). A visão
// "Concluído" é a exceção necessária — ali o recorte É o histórico concluído, e aplicar a
// exclusão devolveria sempre zero. A contagem levada aparece no rótulo do botão e na
// confirmação, então a diferença entre "visíveis" e "levadas" nunca é silenciosa.
function mvpNotesBulkSelection(){
...
// pelo operador: sem isto, uma pasta chamada "a --> b" fecharia o comentário e o resto do
// cabeçalho viraria conteúdo do documento. Neutraliza a sequência de fechamento e os
// sinais de marcação; não é escape de HTML (esc()), que aqui não se aplica.
function mvpNotesComentarioSeguro(texto){
...
// testado, não uma segunda formatação. O delimitador é comentário HTML: invisível no
// Markdown renderizado e inequívoco para dividir o arquivo de volta em notas (o `---` do
// front matter não serve como separador, seria ambíguo com ele mesmo).
function mvpNotesBulkMarkdown(sel){
  const schema=(S.mvpNotes && S.mvpNotes.schemaVersion)||5;
  const criterio=sel.soAtivas?'ativas':'concluidas';
  ...
  const blocos=sel.itens.map(it=>`<!-- jpwealth:note ${it.ticket} -->\n`+mvpNotesMarkdown(it).trim());
  return cabecalho+'\n'+blocos.join('\n\n')+'\n';
}
// Lote para colar num agente de IA. O PREÂMBULO é a peça crítica: cada Trace Reference
// termina com uma instrução própria e dependente da política da nota — concatenar notas de
// políticas diferentes produziria instruções contraditórias em sequência ("AUTORIZA
// IMPLEMENTAÇÃO" logo abaixo de "está BLOQUEADA"). O preâmbulo estabelece a regra de
// leitura antes de qualquer bloco: nenhuma autorização atravessa de uma nota para outra.
// Nada é removido dos blocos — a governança por nota continua íntegra.
const MVPN_BULK_SEPARADOR='\n\n————————————————————————————————\n\n';
function mvpNotesBulkReferenceBlock(sel){
  const n=sel.itens.length;
  const conta=p=>sel.itens.filter(it=>it.aiImplementationPolicy===p).length;
  const autorizadas=conta('autonomous_allowed'), somente=conta('analysis_only'), bloqueadas=conta('blocked');
  const preambulo=[
  ...
  return preambulo+MVPN_BULK_SEPARADOR+sel.itens.map(mvpNotesReferenceBlock).join(MVPN_BULK_SEPARADOR)+'\n';
}
// Rótulo vivo: o número no botão é o que será levado, não o que está na tela. Some quando
// não há nada a levar — um botão "Copiar 0" seria ruído.
function renderMvpNotesBulkActions(){
  const host=mvpn('mvpNotesBulkActions'); if(!host) return;
  const sel=mvpNotesBulkSelection();
  const n=sel.itens.length;
  host.hidden=n===0;
  if(n===0) return;
...
// individual (mvpNotesExportMarkdown): um lote que mistura estado salvo com edição
...
function mvpNotesBulkCopy(btn){
  ...
  const sel=mvpNotesBulkSelection();
  if(!sel.itens.length){
    mvpNotesFlashCopyFeedback(btn,mvpNotesBulkVazioMsg(sel),true);
    return null;
  }
...
function mvpNotesBulkExport(){
  const live=mvpn('mvpNotesExportLive');
  if(mvpNotesBulkGuard('mvpNotesExportLive')) return null;
  const sel=mvpNotesBulkSelection();
  if(!sel.itens.length){
    const msg=mvpNotesBulkVazioMsg(sel);
    if(live) live.textContent=msg;
    alert(msg);
    return null;
  }
  const n=sel.itens.length;
  // Confirmação nativa: é o mesmo mecanismo que o módulo já usa para toda decisão de
  ...
  const nome=mvpNotesBulkFilename(sel);
  // dgDownloadViaAnchor é o download endurecido do projeto (âncora no DOM + revogação
...
function renderMvpNotesList(){
  ...
  // O rótulo trunca com reticências quando a coluna é estreita (nome de pasta longo +
  // ações em massa ao lado); o title preserva o nome inteiro no ponteiro.
  const titleEl=mvpn('mvpNotesViewTitle');
  if(titleEl){ const rotulo=mvpNotesViewLabel(); titleEl.textContent=rotulo; titleEl.title=rotulo; }
  // Antes do early return de #mvpNotesList: o rótulo com a contagem precisa acompanhar
...
function bindMvpNotesDrawer(){
  ...
  // ---- ações em massa sobre o recorte visível (JPW-436587) ----
  const bulkCopy=mvpn('mvpNotesBulkCopyBtn');
  if(bulkCopy) bulkCopy.addEventListener('click',()=>mvpNotesBulkCopy(bulkCopy));
  const bulkExport=mvpn('mvpNotesBulkExportBtn');
  if(bulkExport) bulkExport.addEventListener('click',mvpNotesBulkExport);
