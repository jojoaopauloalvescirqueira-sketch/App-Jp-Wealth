...
(function(root){
  ...
  function render(){
    ...
    const a=model.account;el('forexV11Overview').innerHTML=`<div class="fx-engine-status"><strong>${safe(model.executionEligibility.status)} — elegibilidade normativa</strong><p>${model.canRecord?'O registro factual permanece acessível, sujeito à integridade da gravação.':'Registro indisponível nesta versão do agregado; preserve a base.'} ${safe(fx.policy.version)}</p><button type="button" data-fx-route="params">Registrar observações / ver parâmetros</button></div><div class="fx-engine-cards">
